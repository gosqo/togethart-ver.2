document.addEventListener("DOMContentLoaded", async function () {
    ...
    async function getSubscriptionExpiryStatus(memberId, target) {
      try {
        const response = await fetch(`/sub/member/${memberId}/subs-info/${target}/IsSubscriptionExpired`);
        if (response.status === 200) {
          const { isSubscriptionExpired } = await response.json();
          return isSubscriptionExpired;
        } else {
          console.error("멤버 구독 만료 상태 가져오기 실패: " + response.status);
        }
      } catch (error) {
        console.error("오류 발생:", error);
      }
    ...
    const isSubscriptionExpired = await getSubscriptionExpiryStatus(memberId, target);

    if(memberAuth == 1 && isSubscriptionExpired == true ){
